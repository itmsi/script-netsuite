...
define(['N/search'], (search) => {
    ...
    function post(requestBody) {
        ...
        page.data.forEach(so => {
            const soId = so.id;
            const soObj = {
                internalId: soId,
                tranId: so.getValue("tranid"),
                customerId: so.getValue("entity"),
                customerName: so.getText("entity"),
                status: so.getText("status"),
                statusId: so.getValue("status"),
                tranDate: formatToISO(so.getValue("trandate")),
                lastModifiedDate: formatToISO(so.getValue("lastmodifieddate")),
                lines: []
            };
